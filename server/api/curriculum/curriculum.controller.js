...
module.exports = {
  addCurriculum: (req, res) => {
    if (req.body) {
      if (req.body.curriculumType == "SAME" && !req.body.curriculumName)
        ...
        curriculum
          .createCurriculumForGivenDepartments(models, req.body)
          .then(output => {
            ...
            if (result) {
              ...
              if (error == "IS_ALREADY_EXISTS")
                res
                  .status(400)
                  .json({ data: [], message: "IS_ALREADY_EXISTS" });
              else {
                res
                  .status(500)
  ...
  getCurriculum: (req, res) => {
    if (req.body) {
      curriculum.getAllCurriculum(models, req.body).then(output => {
        let result = output.result;
        let error = output.error;
        if (result)
  ...
  deleteCurriculum: (req, res) => {
    if (req.body) {
      curriculum.deleteCurriculum(models, req.body).then(output => {
        let result = output.result;
        let error = output.error;
        if (result) {
  ...
  editCurriculum: (req, res) => {
    if (req.body) {
      curriculum.editCurriculum(models, req.body).then(output => {
        let result = output.result;
        let error = output.error;
        if (result) {
          res.status(200).json({ data: result, message: "SUCCESS_OPERATION" });
  ...
  assignSubjectElective: (req, res) => {
    if (req.body) {
      curriculum.allocateSubjectElective(models, req.body).then(output => {
        let result = output.result;
        let error = output.error;
        if (result) {
          res.status(200).json({ data: result, message: "SUCCESS_OPERATION" });
  ...
  getSemesterListWithNoCurriculum: (req, res) => {
    if (req.body) {
      curriculum.getSemesterList(models, req.body).then(output => {
        let result = output.result;
        let error = output.error;
        if (result) {
          res.status(200).json({ data: result, message: "ROWS_FOUND" });
  ...
  getDepartmentList: (req, res) => {
    if (req.body) {
      curriculum.getDepartmentList(models, req.body).then(output => {
        let result = output.result;
        let error = output.error;
        if (result.length > 0) {
          res
  ...
  freezeCurriculum: (req, res) => {
    if (req.body) {
      curriculum.freezeCurriculum(models, req.body).then(output => {
        let result = output.result;
        let error = output.error;
        if (result)
          res.status(200).json({
            data: { No_OF_ROWS_UPDATED: result },
            message: "SUCCESS_OPERATION"
          });
        else {
          console.log("Error: ", error);
      ...
      res.status(400).json({
        error: "Missing Paramters: curriculum Id",
        message: "IS_INVALID_INPUT_FORM"
      });
    }
  }
