  ...
  let user = connection.define(
    ...
    },
    {
      classMethods: {
        ...
        findUserByEmail: function(models, email_id, password) {
        let userDetail = models.user_detail;
          return userDetail
            .update(
              {
            ...
            .then(updatedData => {
              return userDetail.findOne({
                attributes: ["email_id", "password"],
                where: { id: updatedData[0] }
              });
            })
            .catch(error => {
              return { error };
        ...
        findUserByUsername: function(models, username, password) {
          let userDetail = models.user_detail;
            return userDetail
              .update(
                {
        ...
        getUserRole: function(models, userId, cb) {
          ...
          admin
            .findOne({ attributes: ["id"], where: { user_detail_id: userId } })
            .then(resultAdmin => {
              if (resultAdmin) {
                role.push("admin");
              }
